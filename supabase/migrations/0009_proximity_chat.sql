...
BEGIN
  ...
  IF v_existing_chat_id IS NOT NULL THEN
    ...
    -- Update center position (average of all members)
    SELECT AVG(pcm2.x), AVG(pcm2.y) INTO v_center_x, v_center_y
    FROM (
      SELECT pcm.user_id, 
        COALESCE(
          (SELECT x FROM (
            SELECT x FROM proximity_chat_members pcm3
            JOIN proximity_chats pc3 ON pc3.id = pcm3.chat_id
            WHERE pcm3.user_id = pcm.user_id
            LIMIT 1
          ) sub), p_x
        ) as x,
        COALESCE(
          (SELECT y FROM (
            SELECT y FROM proximity_chat_members pcm3
            JOIN proximity_chats pc3 ON pc3.id = pcm3.chat_id
            WHERE pcm3.user_id = pcm.user_id
            LIMIT 1
          ) sub), p_y
        ) as y
      FROM proximity_chat_members pcm
      WHERE pcm.chat_id = v_existing_chat_id
    ) pcm2;
    
    -- For simplicity, use current position as center
    -- In a more sophisticated implementation, we'd track member positions
